...
void GameOfLifeParralel(int n, int stepLimit, int rank, int numOfProcesses);
...
int main(int argc, char *argv[]) {
    ...
    GameOfLifeParralel(imgSize, stepLimit, rank, numOfProcesses);
...
void GameOfLifeParralel(int n, int stepLimit, int rank, int numOfProcesses) {
    int minRow = (n - 2) * rank / numOfProcesses + 1;
    int maxRow = (n - 2) * (rank + 1) / numOfProcesses;
    for (int step = 1; step < stepLimit; ++step) {
        ...
        if(rank == 0) {
            ...
            MPI_Irecv(&rec, 1, MPI_INT, rank+1, 0, MPI_COMM_WORLD, &reqtab[1]);
            ...
            MPI_Irecv(&rec2, 1, MPI_INT, rank+1, 0, MPI_COMM_WORLD, &reqtab[3]);
